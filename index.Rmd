...
knitr::opts_chunk$set(echo = FALSE) # we set this to false, so that by default our code will not show up in our finished document

library(palmerpenguins) #penguin data
library(tidyverse) # data tidying and plots
library(gt) # easy table presentation
library(gtsummary) # readymade tidy analysis summaries
...
table <- df %>%
  select(- island, - sex) %>% # we can remove variables with the `-` symbol in select()
  na.omit() %>% # removing na to make tidier data summary
  dplyr::rename(`Bill Length (mm)`= bill_length_mm ,
                `Bill Depth(mm)` = bill_depth_mm ,
                `Flipper Length(mm)` = flipper_length_mm ,
                `Body Mass(g)`= body_mass_g ) %>%
...
Edit the code chunks below to present 2 summary tables. The first by Island, the second by Sex.
