...
```{r, echo=FALSE }
# Setting model
m1 <- lm(scale(body_mass_g) ~ scale(flipper_length_mm), df)
...
modelsummary(model_list,
             ...
             stars = TRUE,
             gof_map = c("nobs", "df", "r.squared", "adj.r.squared", "F"),
             output = "html")
...
* Model fit is significant (*F* (`r model_fit$df.residual`, `r model_fit$df`) = `r round(model_fit$statistic, 2)`, *p* < `r plyr::round_any(model_fit$p.value , .001, ceiling)`), with `r round(model_fit$adj.r.squared * 100, 2)`% of model variance explained by the $R^2_{adj}$.
...
```{r, echo=FALSE }
# Setting model
m2 <- lm(scale(body_mass_g) ~ scale(flipper_length_mm) * species, df)
...
* Flipper Length when accounting for `Species` is a significant predictor of Bodymass in Penguins ($\beta$ = `r round(tidy_1$estimate[2], 2)`, *p* < `r plyr::round_any(tidy_1$p.value[2] , .001, ceiling)`).
* Model fit is significant (*F* (`r model_fit$df.residual`, `r model_fit$df`) = `r round(model_fit$statistic, 2)`, *p* < `r plyr::round_any(model_fit$p.value , .001, ceiling)`), with `r round(model_fit$adj.r.squared * 100, 2)`% of model variance explained by the $R^2_{adj}$.
